...
#manually fix bad longitude values
load("pb_ru.RData")  #some longitude values also incorrect in polar bear effort but grid cell IDs look ok
load("old_grid_df.RData")  #has "old" grid IDs,lat,longs
I.intersect = as.vector(gIntersects(Effort_sp,gUnionCascaded(grid_large),byid=TRUE))
Which_out = which(I.intersect==0)

Grid_IDs = pb_ru[Which_out,"cell"]
Lon_replace = rep(0,length(Grid_IDs))
for(i in 1:length(Grid_IDs))Lon_replace[i]=Grid_df$longitude[which(Grid_df$cell==Grid_IDs[i])]
Effort[Which_out,"Lon"] = Lon_replace
spdf <- SpatialPoints(coords = Effort[,5:6],
                      proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
Effort_sp = spTransform(spdf,CRS(laea_180_proj))
...
save(HO_out,file="Haulout_dists_russia.RData")
...
save(Data_rus,file = "CHESS_russia_Data_TMB_Apr2021.RData")
